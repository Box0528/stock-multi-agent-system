...
## 时间约束（严格执行，以具体日期为准）
- 今天是 {today}
- 近7天截止日期：{cutoff_7d}（含）之后的新闻 → 写入"核心新闻摘要（近7天内）"
- 历史参考截止日期：{cutoff_30d}（含）到 {cutoff_7d} 之间的新闻 → 写入"历史参考（7-30天）"
- {cutoff_30d} 之前的新闻：直接降为 C 级，不得写入报告
- 判断时必须比较新闻的具体日期字符串，不得估算
...
### 核心新闻摘要（近7天内，{cutoff_7d} 之后）
（只写新闻日期 ≥ {cutoff_7d} 的A/B级新闻；无则写"近7天内无重要新闻"）
1. [A级] 标题（YYYY-MM-DD，来源）— 一句话：对股价的实际影响
2. ...

### 历史参考（{cutoff_30d} ~ {cutoff_7d}，非近期）
（只写新闻日期在 {cutoff_30d} 到 {cutoff_7d} 之间的A/B级新闻；无则省略本节）
1. [B级] 标题（YYYY-MM-DD，来源）— 简要说明【历史参考，非近期】
