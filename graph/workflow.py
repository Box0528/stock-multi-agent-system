...
class ResearchState(TypedDict):
    ...
    technical_grounding:  Annotated[float, _keep_last_float]  # Tool Receipts 数据核实率（None→1.0）
    news_grounding:       Annotated[float, _keep_last_float]
    sector_grounding:     Annotated[float, _keep_last_float]
...
def memory_load_node(state: ResearchState, config: RunnableConfig) -> dict:
    bus = get_event_bus(config)
    stock_name = state["stock_name"]
    bus.emit_progress("supervisor", "running", f"🧠 加载历史记忆：{stock_name}")
    ...
    try:
        ...
        if mem["has_history"]:
            ...
            bus.emit_progress("supervisor", "running", "🧠 首次分析，无历史记忆")
            context = ""
        ...
        bus.emit_progress("supervisor", "running", "🧠 记忆加载失败，继续分析")
...
def parallel_analysts_node(state: ResearchState, config: RunnableConfig) -> dict:
    ...
    return {
        ...
        "technical_grounding":  reports["technical"].grounding_score if reports["technical"].grounding_score is not None else 1.0,
        "news_grounding":       reports["news"].grounding_score if reports["news"].grounding_score is not None else 1.0,
        "sector_grounding":     reports["sector"].grounding_score if reports["sector"].grounding_score is not None else 1.0,
        "reasoning_traces":     "\n\n".join(traces),
...
def supervisor_node(state: ResearchState, config: RunnableConfig) -> dict:
    ...
    output = run_supervisor(
        ...
        technical_grounding=state.get("technical_grounding", 1.0),
        news_grounding=state.get("news_grounding", 1.0),
        sector_grounding=state.get("sector_grounding", 1.0),
...
def build_workflow():
    ...
    g.add_edge("memory_load",  "analysts")
