...
def refresh_all_stocks(bus=None, progress_callback=None) -> dict:
    """增量更新全部股票（模式一扫描前调用）。
    ...
    total = len(meta_df)
    bus.emit_progress("system", "running", f"📡 开始全量增量更新（{total} 只股票）...")
    ...
    try:
        ...
        for i, row in meta_df.iterrows():
            ...
            try:
                ...
                failed += 1
                logger.warning("更新失败 %s：%s", code, e)

            if (i + 1) % 50 == 0:
                bus.emit_progress("system", "running",
                    f"📡 数据更新进度：{i+1}/{total}（更新{updated} 跳过{skipped} 失败{failed}）")
                # 每50只重登一次防止长连接断开
                try:
                    _logout_baostock()
                    _login_baostock()
