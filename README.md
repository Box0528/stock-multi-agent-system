...
- **Tool Receipts**：工具调用的原始返回作为收据保留，报告生成后以确定性程序核验数字声明的来源可查性，`grounding_score` 注入 Supervisor prompt 参与最终研判
- **四层向量记忆**：ChromaDB 存储历史预测、板块轮动、风控信号、Agent 行为教训，教训在下次分析时自动注入对应 Agent 的 system prompt
- **Reflection**：workflow 结束后在独立线程拉取实时价格，与上次建议对比计算区间涨跌幅，归因结论和行为修正建议写回 ChromaDB，下次分析时自动加载
...
```mermaid
flowchart TD
    A1[模式一: 全市场量化选股] --> A2[LLM 精选 Top 5]
    B1[模式二: 用户指定股票代码]
